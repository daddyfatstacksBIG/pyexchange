...
class Order:
    ...
    def remaining_buy_amount(self) -> Wad:
        return (self.remaining_amount *
                self.price if self.is_sell else self.remaining_amount)

    @property
    def remaining_sell_amount(self) -> Wad:
        return (self.remaining_amount
                if self.is_sell else self.remaining_amount * self.price)
...
class BittrexApi(PyexAPI):
    ...
    def get_pair(self, pair: str):
        assert isinstance(pair, str)
        return next(
            filter(lambda symbol: symbol["MarketName"] == pair,
                   self.get_markets()))

    def get_balances(self):
        return self._http_authenticated_request(
            "GET", "/api/v1.1/account/getbalances", {})["result"]
    ...
    def place_order(self, pair: str, is_sell: bool, price: Wad,
        ...
        params = {"quantity": str(amount), "rate": str(price), "market": pair}

        order_type = "selllimit" if is_sell else "buylimit"

        self.logger.info(
            f"Placing order ({order_type}, amount {params['quantity']} of {pair},"
            f" price {params['rate']})...")
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        result = self._http_authenticated_request("GET",
    ...
    def get_trades(self, pair: str, page_number: int = 1) -> List[Trade]:
        ...
        return list(
            map(
                lambda item: Trade(
                    trade_id=item["OrderUuid"],
                    timestamp=int(
                        dateutil.parser.parse(item["TimeStamp"] + "Z").
                        timestamp()),
                    pair=item["Exchange"],
                    is_sell=item["OrderType"] == "LIMIT_SELL",
                    price=Wad.from_number(item["PricePerUnit"]),
                    amount=Wad.from_number(item["Quantity"] - item[
                        "QuantityRemaining"]),
                ),
                result,
            ))
    ...
    def get_all_trades(self, pair: str, page_number: int = 1) -> List[Trade]:
        ...
        result = self._http_request("GET", "/api/v1.1/public/getmarkethistory",
                                    params)["result"]
    ...
    def _http_authenticated_request(self, method: str, resource: str,
        ...
        signature = hmac.new(self.secret_key.encode(), url.encode(),
