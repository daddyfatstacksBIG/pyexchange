...
class Order:
    ...
    def remaining_sell_amount(self) -> Wad:
        return (
            ...
            if self.is_sell
            else self.remaining_amount * self.price
        )
...
class BittrexApi(PyexAPI):
    ...
    def get_pair(self, pair: str):
        assert isinstance(pair, str)
        return next(
            filter(lambda symbol: symbol["MarketName"] == pair, self.get_markets())
        )
    ...
    def get_orders(self, pair: str) -> List[Order]:
        ...
        orders = self._http_authenticated_request(
            "GET", "/api/v1.1/market/getopenorders", params
        )["result"]
    ...
    def get_trades(self, pair: str, page_number: int = 1) -> List[Trade]:
        ...
        result = self._http_authenticated_request(
            "GET", "/api/v1.1/account/getorderhistory", params
        )["result"]
    ...
    def _http_authenticated_request(self, method: str, resource: str, params: dict):
        ...
        signature = hmac.new(
            self.secret_key.encode(), url.encode(), hashlib.sha512
        ).hexdigest()
