...
class OKEXApi:
    ...
    def get_trades(self, pair: str, page_number: int = 1) -> List[Trade]:
        ...
        result_part_filled = self._http_get(
            f"/api/spot/v3/orders",
            f"state=1&instrument_id={pair}",
            requires_auth=True,
            has_cursor=False,
        )
        result_filled = self._http_get(
            "/api/spot/v3/orders",
            f"state=2&instrument_id={pair}",
            requires_auth=True,
            has_cursor=False,
        )
    ...
    def get_all_trades(self, pair: str, page_number: int = 1) -> List[Trade]:
        ...
        result = self._http_get(
            f"/api/spot/v3/instruments/{pair}/trades",
            f"symbol={pair}",
            requires_auth=False,
            has_cursor=False,
        )
